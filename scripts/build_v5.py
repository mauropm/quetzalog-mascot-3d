"""Quetzalog mascot - fifth pass (v5): sphinx-sit body plan matching image2.
...
def build_head():
    ...
    # crown mass the crest grows out of
    add_ellipsoid((0, -4, 74), (14.0, 12.5, 12.5), name="crest_base")
    # brow band over the eyes
    add_box((0, -19.0, 77.0), (28.0, 12.0, 8.5), 4.0, name="brow")
    # snout
    loft([(58.0, 7.0, 7.4, -30.0), (61.5, 8.0, 8.2, -30.0),
          (67.0, 8.0, 8.2, -30.0), (71.0, 6.6, 6.8, -30.0),
          (73.0, 4.2, 4.4, -30.0)], n=32, power=2.6, name="snout")
    reg(76, "Body_Teal", t_ellip((0, -30, 66), (9.2, 9.4, 9.2)))
    # cream cheeks + jaw
    for sx in (-1, 1):
        add_box((sx * 9.4, -25.5, 60.0), (13.0, 13.0, 13.0), 5.6, name="cheek")
    add_box((0, -27.5, 56.0), (18.5, 12.0, 9.5), 4.5, name="chin")
    reg(72, "Cream", t_ellip((9.4, -26.0, 60.0), (9.4, 11.2, 9.2)))
    reg(72, "Cream", t_ellip((-9.4, -26.0, 60.0), (9.4, 11.2, 9.2)))
    reg(72, "Cream", t_ellip((0, -27.5, 56.0), (10.8, 10.4, 7.4)))
    # nostrils
    for sx in (-1, 1):
        add_ellipsoid((sx * 3.7, -37.4, 68.0), (1.3, 1.7, 1.3), col=DETAIL,
                      name="nostril")
        reg(95, "Eye_Pupil", t_ellip((sx * 3.7, -37.5, 68.0), (1.7, 2.1, 1.7)))
    # red forehead crest
    add_box((0, -21.0, 80.0), (8.0, 10.0, 12.5), 2.4, col=DETAIL, name="crest_red")
    add_box((0, -20.0, 86.0), (5.4, 7.0, 5.0), 1.5, col=DETAIL, name="crest_red2")
    reg(80, "Red", t_ellip((0, -21, 81.5), (6.6, 7.4, 9.5)))


def build_eyes():
    for sx in (-1, 1):
        ec = Vector((sx * 9.5, -28.5, 68.0))
        add_ellipsoid(ec, (7.6, 7.0, 8.2), col=DETAIL, name="sclera")
        I = ec + Vector((sx * 0.5, -4.8, -0.7))
        add_ellipsoid(I, (5.2, 4.3, 5.6), col=DETAIL, name="iris")
        P = ec + Vector((sx * 0.7, -6.9, -0.9))
        add_ellipsoid(P, (2.7, 2.3, 2.9), col=DETAIL, name="pupil")
        H = P + Vector((-sx * 1.2, -1.4, 1.3))
        add_ellipsoid(H, (1.4, 1.4, 1.4), col=DETAIL, name="hilite")
        reg(100, "Eye_White", t_ellip(ec, (8.2, 7.6, 8.8)))
        reg(101, "Eye_Iris", t_ellip(I, (5.6, 4.7, 6.0)))
        reg(102, "Eye_Pupil", t_ellip(P, (3.1, 2.7, 3.3)))
        reg(103, "Eye_White", t_ellip(H, (1.7, 1.7, 1.7)))
...
# ================================================================= CREST =====
def build_crest():
    # large swept-back rosette; plane tilted ~48 deg rearward
    C = Vector((0, 2.0, 75.0))
    X = Vector((1, 0, 0))
    pal = ["Green", "Blue", "Red", "Yellow", "Orange", "Purple",
           "Blue", "Green", "Red", "Orange", "Yellow", "Purple"]
    # one coherent fan plane (tilted back), layered by radius/length
    rings = [
        (30.0, 11.0, 19.0, 11.0, 3.4, 13, 0.0),
        (45.0, 12.0, 18.0, 11.5, 3.5, 14, 13.0),
        (62.0, 13.0, 16.0, 12.0, 3.6, 14, 6.0),
        (80.0, 14.0, 13.0, 11.0, 3.4, 14, 0.0),
    ]
    ci = 0
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "quetzalog_v5.blend"))
