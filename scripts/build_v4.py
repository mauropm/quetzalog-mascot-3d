...
def make_leaf(length, width, thickness, name="leaf", us=20, vs=11,
              base_w=0.10, stud=True, stud_r=1.6, stud_h=1.2):
    bm = bmesh.new()
...
def build_body():
    ...
    for zz in (20.0, 27.0, 34.0):
        for xx in (-3.6, 3.6):
            add_cyl((xx, -18.6, zz), (xx, -19.4, zz), 1.9, col=DETAIL,
                    name="belly_stud")
    # chest plate
...
def build_head():
    ...
    add_ellipsoid((0, 5, 73), (15.5, 13.5, 13.5), name="crest_base")
    # brow band over the eyes
    add_box((0, -11.0, 74.5), (30.0, 13.0, 9.0), 4.0, name="brow")
    # crown behind the red crest
    add_box((0, 3.0, 74.0), (28.0, 19.0, 12.0), 5.0, name="crown")
    # snout
    loft([(45.0, 7.6, 8.6, -22.0), (48.5, 8.4, 9.2, -22.0),
          (53.0, 8.6, 9.4, -22.0), (56.5, 7.6, 8.4, -22.0),
          (58.8, 4.6, 5.2, -22.0)], n=32, power=2.6, name="snout")
    reg(76, "Body_Teal", t_ellip((0, -22, 55), (9.6, 10.0, 9.0)))
    # cream cheeks + jaw
    for sx in (-1, 1):
        add_box((sx * 9.6, -14.5, 49.5), (12.5, 13.0, 13.5), 5.5, name="cheek")
    add_box((0, -15.0, 44.0), (20.5, 13.0, 9.5), 4.5, name="chin")
    reg(72, "Cream", t_ellip((9.6, -15.0, 49.5), (9.2, 11.2, 9.4)))
    reg(72, "Cream", t_ellip((-9.6, -15.0, 49.5), (9.2, 11.2, 9.4)))
    reg(72, "Cream", t_ellip((0, -15.0, 43.5), (11.4, 11.0, 7.2)))
    # nostrils
    for sx in (-1, 1):
        add_ellipsoid((sx * 3.9, -29.6, 54.5), (1.2, 1.6, 1.2), col=DETAIL,
                      name="nostril")
        reg(95, "Eye_Pupil", t_ellip((sx * 3.9, -29.7, 54.5), (1.6, 2.0, 1.6)))
    # red forehead crest
...
def build_eyes():
    for sx in (-1, 1):
        ec = Vector((sx * 10.2, -15.5, 61.0))
        add_ellipsoid(ec, (8.4, 7.6, 9.0), col=DETAIL, name="sclera")
        I = ec + Vector((sx * 0.6, -5.0, -0.8))
        add_ellipsoid(I, (5.6, 4.5, 6.0), col=DETAIL, name="iris")
        P = ec + Vector((sx * 0.8, -7.3, -1.0))
        add_ellipsoid(P, (2.9, 2.4, 3.1), col=DETAIL, name="pupil")
        H = P + Vector((-sx * 1.3, -1.6, 1.4))
        add_ellipsoid(H, (1.6, 1.6, 1.6), col=DETAIL, name="hilite")
        reg(100, "Eye_White", t_ellip(ec, (9.0, 8.2, 9.6)))
        reg(101, "Eye_Iris", t_ellip(I, (6.0, 4.9, 6.4)))
        reg(102, "Eye_Pupil", t_ellip(P, (3.3, 2.8, 3.5)))
        reg(103, "Eye_White", t_ellip(H, (1.9, 1.9, 1.9)))
...
def build_legs():
    for sx in (-1, 1):
        ...
        loft([(0.0, 6.4, 8.4, 0.0), (3.5, 7.8, 10.4, 0.0),
              (8.5, 8.4, 10.8, 0.0), (13.5, 6.8, 8.6, 0.0)], n=28, power=3.0,
        ...
        loft([(0.0, 6.2, 8.2, 0.0), (3.5, 7.6, 10.2, 0.0),
              (8.5, 8.2, 10.6, 0.0), (13.5, 6.6, 8.4, 0.0)], n=28, power=3.0,
             name="bfoot", loc=(sx * 19.5, 16.0, 0))
        for i in (-1, 0, 1):
            c = Vector((sx * 21.5 + i * 4.6, 25.0, 5.8))
            add_ellipsoid(c + Vector((sx * 1.6, 4.8, 0.8)), (3.0, 4.8, 3.0),
                          col=DETAIL, name="claw")
            reg(90, "Cream", t_ellip(c + Vector((sx * 1.0, 2.4, 0.6)),
                                     (3.4, 5.4, 3.4)))
...
def build_crest():
    ...
    # (tilt_deg, R, L, W, T, N, phase_deg)  tips form a ~28 mm halo at z=74
    rings = [
        (12.0, 11.5, 17.0, 10.5, 3.2, 12, 0.0),
        (30.0, 12.5, 15.5, 11.0, 3.3, 13, 14.0),
        (50.0, 13.5, 15.0, 11.0, 3.4, 14, 0.0),
        (70.0, 14.5, 12.5, 10.5, 3.3, 14, 13.0),
        (92.0, 15.5, 10.0, 10.0, 3.2, 14, 6.0),
    ]
    ci = 0
...
def build_wings():
    ...
    for sx in (-1, 1):
        arm = [Vector((sx * 16, 6, 51)), Vector((sx * 25, 9, 55)),
               Vector((sx * 33, 12, 59)), Vector((sx * 40, 15, 62))]
        for i in range(3):
            add_cyl(arm[i], arm[i + 1], 6.4 - i * 0.5, name="warm")
        add_ellipsoid(arm[0], (7.2, 6.8, 6.8), name="shoulder")
        add_ellipsoid(arm[-1], (5.8, 5.6, 5.6), name="wrist")
        ci = 0
        n = 8
        for i in range(n):
            t = i / (n - 1.0)
            ...
            ang = D2R(-52 + 42 * t)
            d = Vector((sx * math.cos(ang), 0.26, math.sin(ang))).normalized()
            L = 18 + 6 * math.sin(math.pi * (0.20 + 0.70 * t))
            leaf(base, base + d * L, 13.5, 3.4, pal[ci % len(pal)],
                 up_hint=(0, 0.76, 0.65))
            ci += 1
        for i in range(6):
            t = i / 5.0
            ...
            ang = D2R(-62 + 36 * t)
            d = Vector((sx * math.cos(ang), 0.46, math.sin(ang))).normalized()
            L = 12 + 5 * math.sin(math.pi * (0.2 + 0.7 * t))
            leaf(base, base + d * L, 12.0, 3.3, pal[(ci + 2) % len(pal)],
                 up_hint=(0, 0.76, 0.65))
            ci += 1


# ================================================================== TAIL =====
TAIL_CTRL = [(0, 14, 21), (4, 25, 20), (10, 35, 20), (17, 43, 21),
             (25, 49, 22), (32, 52, 24), (38, 51, 26), (42, 46, 28)]
...
def build_tail():
    ...
    radii = [9.5 + (2.6 - 9.5) * (i / (N - 1.0)) ** 0.8 for i in range(N)]
    ...
    for i in range(3, N, 3):
        ...
        for k in range(5):
            ...
            L = (16.0 + 5.0 * math.sin(ang * 1.7)) * (1.0 - 0.40 * i / (N - 1.0))
            leaf(base, base + radial * L + seg * (L * 0.40), 9.5, 3.2,
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "quetzalog_v4.blend"))
