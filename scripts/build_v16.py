"""Quetzalog mascot - v14: close the through-hole under the head.
...
VOXEL = 0.45
# head group transform (build coords): pivot sits at the head's underside so
# the jaw stays put while the skull grows up and out
HEAD_PIVOT = Vector((0.0, -24.0, 52.0))
HEAD_SCALE = Vector((1.16, 1.16, 1.06))
TARGET_H = 100.0
D2R = math.radians
...
# The head is built at its own scale and then blown up as a group just before
# the join, so its parts stay consistent with each other.  Geometry coordinates
# are authored in HEAD space; HEAD_XFORM is (pivot, scale) in build coords.
HEAD_XFORM = None


def headreg(prio, mat, test):
    """Register a region whose coordinates are in HEAD space.  The test is
    wrapped so it still matches after the head group transform is applied: a
    query point p in build space maps back to head space by undoing the scale
    about the pivot."""
    if HEAD_XFORM is None:
        reg(prio, mat, test)
        return
    P, S = HEAD_XFORM

    def wrapped(p):
        return test(Vector((P.x + (p.x - P.x) / S.x,
                            P.y + (p.y - P.y) / S.y,
                            P.z + (p.z - P.z) / S.z)))
    reg(prio, mat, wrapped)


def apply_xform(objs, P, S):
    """Scale a group of objects about a pivot, in world space, by editing the
    mesh.  Editing vertices rather than object transforms means it works for
    lofts (mesh already in world coords, identity matrix) and for primitives
    (local mesh + location + rotation) alike."""
    for o in objs:
        mw = o.matrix_world.copy()
        inv = mw.inverted()
        for v in o.data.vertices:
            w = mw @ v.co
            v.co = inv @ Vector((P.x + S.x * (w.x - P.x),
                                 P.y + S.y * (w.y - P.y),
                                 P.z + S.z * (w.z - P.z)))
        o.data.update()
...
def build_body():
    ...
    reg(62, "Cream", t_capsule((0, -19.6, 34.0), (0, -19.0, 52.0), 4.6))
...
def build_head():
    ...
    # nape: the neck's back (y -2) and the crest base's underside (z 60) left a
    # second through-hole behind the neck.  Hidden by the mane, but a hole is
    # a hole - fill it.
    add_ellipsoid((0, -3.0, 55.0), (9.5, 9.5, 9.0), name="nape")
    ...
    # snout: ONE volume.  Its LOWEST section used to sit at build z 47, which
    # put the jaw's underside at final z 44.5 -- only 3.5 mm above the top of
    # the torso (final z 41).  The muzzle therefore had no visible neck beneath
    # it and read as a fat double chin.  Measured off views/left.png the
    # reference's jaw underside is at final z ~50 with the shoulder at ~39, so
    # the whole underside is raised 5 mm and the muzzle is made correspondingly
    # shallower while its width and its top edge stay exactly where they were.
    loft([(50.0, 7.7, 6.6, -29.5), (53.5, 8.3, 8.3, -31.2),
          (57.5, 8.6, 8.8, -33.5), (61.5, 8.4, 8.8, -34.3),
          (65.0, 7.0, 7.4, -33.0), (67.0, 4.4, 4.6, -30.5)],
         n=32, power=2.6, name="snout")
    headreg(76, "Body_Teal", t_ellip((0, -33.5, 60.5), (9.2, 12.5, 7.0)))
    ...
    # against a cylinder
    add_box((0, -40.0, 62.0), (12.2, 9.0, 9.0), 2.5, name="nose")
    # cheek pads flanking the snout, tucked up beside the eyes
    for sx in (-1, 1):
        add_box((sx * 8.4, -28.0, 63.0), (9.8, 9.6, 9.4), 4.6, name="cheek")
        headreg(72, "Cream", t_ellip((sx * 8.4, -28.5, 62.5), (8.2, 9.6, 8.0)))
    ...
    headreg(74, "Cream", t_ellip((0, -34.5, 54.5), (9.6, 12.0, 9.0)))
    ...
    headreg(63, "Cream", t_capsule((0, -14.0, 43.0), (0, -31.0, 53.5), 8.2))
    # nostrils: tall ovals on the nose block's front face
    for sx in (-1, 1):
        headreg(95, "Eye_Pupil", t_ellip((sx * 3.5, -43.4, 62.0), (1.6, 2.2, 2.4)))
    # red forehead crest
    ...
    headreg(80, "Red", t_ellip((0, -22, 80.5), (6.6, 7.4, 9.5)))
...
def build_eyes():
    ...
    for sx in (-1, 1):
        ...
        H = ec + hd * 6.9
        headreg(103, "Eye_White", t_ellip(H, (1.4, 1.4, 1.4)))


# Throat fill.  Section 0 sits inside the neck (front y -20.0, behind the
# neck's own front wall at -19.7) and the fill only turns forward at the CUT.
#
# CRITICAL: every section must reach BACK far enough to overlap the neck.  An
# earlier version of this ramp only projected forward (small hy, cy near the
# snout), so at |x| > 6 its cross-section was a thin plate floating in front of
# the neck with empty space behind it -- a genuine through-hole, and it rendered
# as a mug-handle under the head.  Keeping cy well back with hy growing forward
# means every section's back edge is buried in the neck's volume.
THROAT = [
    # Below the jaw the fill is kept INSIDE the neck's own front wall (y -19.6
    # to -20.7) so it adds no forward mass.  A ramp that starts drifting
    # forward down here is exactly what produced the double chin: the surface
    # left the neck early and swept forward in one long convex curve, so the
    # head and the neck merged instead of meeting.
    (42.0, 10.0, 10.0, -10.0),   # front -20.0   (inside the neck)
    (45.0, 9.4, 9.2, -11.0),     # front -20.2   (inside the neck)
    (47.5, 8.7, 8.7, -11.8),     # front -20.5   (inside the neck)
    (49.0, 8.3, 8.3, -12.2),     # front -20.5   (inside the neck)
    # THE CUT.  The fill turns forward here, once, over ~1.5 mm of height.
    # Because the neck's front below is vertical and this ramp leaves it
    # abruptly, the two meet in a corner instead of a curve, and the jaw's
    # underside overhangs the neck.  Voxel remesh rounds that corner to ~1 mm,
    # which is the "soft-edged stylised cut" the reference has.
    (50.5, 8.0, 9.0, -17.5),     # front -26.5
    (52.0, 7.8, 11.5, -20.5),    # front -32.0
    (54.0, 7.5, 13.0, -20.0),    # front -33.0
    # ...then it just carries the fill up to the head, closing the slot behind
    # the snout (this is the part that fixed the pass-12 through-hole).
    (57.0, 7.2, 13.0, -20.5),    # front -33.5
    (59.5, 7.0, 11.5, -21.5),    # front -33.0
]
...
def main():
    ...
    build_neck()
    global HEAD_XFORM
    HEAD_XFORM = (HEAD_PIVOT, HEAD_SCALE)
    _before = set(bpy.data.objects)
    build_head()
    build_eyes()
    _head_objs = [o for o in bpy.data.objects if o not in _before]
    apply_xform(_head_objs, HEAD_PIVOT, HEAD_SCALE)
    for _o in _head_objs:
        if _o.name.startswith(("eyeball", "head", "snout", "brow", "temple", "cheek", "nose")):
            _bb = [Vector(c) for c in _o.bound_box]
            _mn = Vector((min(v.x for v in _bb), min(v.y for v in _bb), min(v.z for v in _bb)))
            _mx = Vector((max(v.x for v in _bb), max(v.y for v in _bb), max(v.z for v in _bb)))
            _mn = _o.matrix_world @ _mn; _mx = _o.matrix_world @ _mx
            print(f"  AFTER  {_o.name:16s} y {_mn.y:7.2f}..{_mx.y:7.2f}  z {_mn.z:6.2f}..{_mx.z:6.2f}")
    build_legs()
    build_mane()
    build_wings()
    build_tail()
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "quetzalog_v16.blend"))
