"""Quetzalog mascot - eighth pass (v8): soft cartoon anatomy.
...
def build_head():
    ...
    # snout
    loft([(51.5, 6.8, 7.4, -30.0), (55.0, 7.6, 8.2, -32.5),
          (60.0, 7.6, 8.2, -33.5), (64.5, 6.4, 6.9, -32.0),
          (67.0, 4.1, 4.3, -30.0)], n=32, power=2.6, name="snout")
    reg(76, "Body_Teal", t_ellip((0, -32, 60), (8.8, 9.8, 8.8)))
    # cream cheeks + jaw
    for sx in (-1, 1):
    ...
    # throat: fills the notch between the jaw overhang and the neck
    add_ellipsoid((0, -21.5, 53.0), (8.2, 6.4, 6.2), name="throat")
    # nostrils
    for sx in (-1, 1):
        add_ellipsoid((sx * 3.6, -40.0, 61.8), (1.3, 1.7, 1.3), col=DETAIL,
                      name="nostril")
        reg(95, "Eye_Pupil", t_ellip((sx * 3.6, -40.1, 61.8), (1.7, 2.1, 1.7)))
    # red forehead crest
...
def build_legs():
    for sx in (-1, 1):
        # front leg: tapered column + rounded shoulder mass that blends the
        # leg into the chest instead of a cylinder butting into the torso
        # the leg widens continuously into the chest, so the shoulder is a
        # transition volume rather than a sphere bolted onto the torso
        loft([(0.0, 6.6, 7.8, 0.0), (5.0, 7.4, 8.6, 0.0),
              (12.0, 7.8, 9.0, 0.0), (20.0, 8.2, 9.4, 0.2),
              (26.0, 8.8, 9.8, 0.2), (31.0, 9.4, 10.2, -0.4),
              (35.0, 9.0, 10.0, -1.2), (38.0, 7.0, 8.2, -1.2),
              (40.5, 4.2, 5.0, -1.0)],
             n=30, power=2.5, name="fleg", loc=(sx * 9.5, -11.0, 0))
        ...
        # rear haunch: tapered leg + rounded hip mass blending into the flank
        # haunch widens continuously into the flank for a soft cartoon hip
        loft([(6.0, 8.0, 9.0, 0.0), (12.0, 8.8, 9.8, 0.0),
              (19.0, 9.2, 10.2, -0.2), (24.0, 9.8, 10.6, -0.6),
              (29.0, 9.8, 10.6, -1.0), (33.0, 8.8, 9.8, -1.2),
              (36.5, 6.8, 7.8, -1.2), (39.5, 4.2, 5.0, -1.0)],
             n=30, power=2.5, name="bleg", loc=(sx * 13.5, 12.5, 0))
...
def build_spine():
    """Dorsal ridge: short roof-tile feathers rooted on the back surface.

    They flow down-and-back from the nape to the tail base and lie close to
    the body, so they read as a continuation of the mane rather than as a
    floating ladder bridging the tail back up into the upper back."""
    pal = ["Green", "Blue", "Red", "Orange", "Yellow", "Purple"]
    # (y, z) roots measured on the back surface, nape -> tail base
    pts = [(0.0, 51.0), (3.2, 47.0), (5.5, 43.0), (8.0, 40.0),
           (10.5, 37.0), (12.8, 34.0), (14.9, 31.0), (16.5, 28.0),
           (17.6, 25.0), (18.2, 22.0)]
    d = Vector((0.0, 0.60, -0.80)).normalized()
    for i, (yy, zz) in enumerate(pts):
        b = Vector((0.0, yy - 1.2, zz))
        L = 13.0 - 0.30 * i
        # wide central tile
        leaf(b, b + d * L, 17.0, 3.4, pal[i % len(pal)],
             up_hint=(1, 0, 0))
        # staggered flanking tiles so the whole back is covered wing to wing
        for sx in (-1, 1):
            b2 = b + Vector((sx * 9.5, -0.8, 2.0))
            leaf(b2, b2 + d * (L * 0.85), 14.0, 3.4,
                 pal[(i + 2 + (0 if sx < 0 else 3)) % len(pal)],
                 up_hint=(1, 0, 0))
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "quetzalog_v8.blend"))
