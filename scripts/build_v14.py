...
# Throat ramp.  Section 0 sits inside the neck (front y -18.9, behind the neck's
# own front wall at -19.8) and the front contour ramps forward to meet the
# snout's underside.
#
# CRITICAL: every section must reach BACK far enough to overlap the neck.  The
# first version of this ramp only projected forward (small hy, cy near the
...
# as a mug-handle under the head.  Keeping cy near -19 with hy growing to 15
# means every section's back edge is buried in the neck's volume.
THROAT = [
    (38.0, 10.4, 15.4, -3.5),    # front -18.9   (inside the neck)
    (40.0, 9.8, 12.0, -8.0),     # front -20.0
    (42.0, 9.2, 9.0, -13.0),     # front -22.0
    (44.0, 8.6, 7.0, -19.0),     # front -26.0
    (45.5, 8.2, 10.5, -19.5),    # front -30.0
    (47.0, 7.8, 15.0, -19.5),    # front -34.5
    # The ramp used to stop at z47.  Above it the snout's BACK (y -22.8 at z50)
    # and the neck's FRONT (y -19.4) left a 3.4 mm slot open all the way up to
    # the head at z55 -- a genuine through-hole, and the visible one.  These
    # sections carry the fill up to the head so the slot is closed.
    (48.5, 7.6, 14.0, -20.5),    # front -34.5
    (50.5, 7.4, 12.0, -21.5),    # front -33.5
    (52.5, 7.2, 10.0, -22.5),    # front -32.5
    (54.5, 7.0, 9.0, -23.0),     # front -32.0
]
...
def main():
    ...
    bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT, "quetzalog_v14.blend"))
